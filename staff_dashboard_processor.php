...
?>
<?php
	///////////////////////// POST ACTION TO REGISTER NEW teacher WHEN REGISTER BUTTON IS CLICKED /////////////////////////////////
	if(isset($_POST['teacher_reg_btn'])){
		$control_access = inject_checker($connection, strtolower($_POST['control_access']));
		$teacher_title = inject_checker($connection, $_POST['teacher_title']);
		$teacher_fullname = inject_checker($connection, $_POST['teacher_fullname']);
		$teacher_address = inject_checker($connection, $_POST['teacher_address']);
		$teacher_phone = inject_checker($connection, $_POST['teacher_phone']);
		$teacher_state = inject_checker($connection, $_POST['teacher_state']);
		$teacher_lga = inject_checker($connection, $_POST['teacher_lga']);
		$teacher_nationality = inject_checker($connection, $_POST['teacher_nationality']);
		$teacher_email = inject_checker($connection, $_POST['teacher_email']);
		$teacher_password = inject_checker($connection, $_POST['teacher_password1']);
		$confirm_password = inject_checker($connection, $_POST['teacher_password2']);
		
		////////////////// ERROR-CHECKING IF SPECIFIED FIELD IS EMPTY ////////////////////
		if(empty($control_access)){
			$error[] = "Error: You Need Control Access to Add Another teacher";
		}
		if(empty($teacher_title)){
			$error[] = "Error: Title required";
		}
		if(empty($teacher_fullname)){
			$error[] = "Error: Fullname required";
		}
		...
		if(!empty($teacher_email)){
			if(single_email_validator($teacher_email)){
		...
		if(empty($error)){
			...
			if(mysqli_num_rows($run_query) == 1){
				$query = "INSERT INTO `teachers`(`title`, `fullname`, `address`, `phone`, `state`, `lga`, `nationality`, `email`, `password`, `date_of_reg`)
						VALUES('$teacher_title', '$teacher_fullname','$teacher_address', '$teacher_phone', '$teacher_state', '$teacher_lga', '$teacher_nationality', '$teacher_email', '$teacher_password', '$date')";
