            ...
            <div class='row header'>
                ...
                <div class='col-md-6'>
                    <?php
						...
						?>
                    <h2 class='text-center'><span class='glyphicon glyphicon-education'></span> Student - SURE FOUNDATION COMPREHENSIVE COLLEGE</h2>
                ...
                <div class='col-md-3 logout'>
                    ...
                    <h4 class='text-right'><a href='student_dashboard.php?logout'><span
        ...
        <section class='row'>
            ...
            <div class='col-md-2 side_nav noprint'>
                <ul class="nav nav-pills nav-stacked">
                    <li role="presentation" class="active"><a href="student_dashboard.php?view_profile"><span
                                class='glyphicon glyphicon-user'></span> View Profile</a></li>
                    <li role="presentation"><a href="student_dashboard.php?check_result"><span
                                class='glyphicon glyphicon-education'></span> Check My Result</a></li>
                    <li role="presentation"><a href="student_dashboard.php?print_slip"><span
                                class='glyphicon glyphicon-print'></span> Print My Slip</a></li>
                    <li role="presentation"><a href="student_dashboard.php?my_subjects"><span
                                class='glyphicon glyphicon-th-list'></span> Specified Subjects</a></li>
                    <li role="presentation"><a href="student_dashboard.php?register_subject"><span
                                class='glyphicon glyphicon-th-list'></span> Register Subjects</a></li>
                    <li role="presentation"><a href="student_dashboard.php?logout"><span
            ...
            <div class='col-md-10'>
                <?php
						...
						elseif (isset($_GET['register_subject'])) {
							...
							if (!isset($student_reg_number) || !isset($student_class)) {
								echo "<p class='text-danger'><b>Student registration number or class not found!</b></p>";
								exit;
							}
						
							// Fetch current term and session
							$query_current_season = "SELECT `current_term`, `current_session` FROM `current_season` LIMIT 1";
							$result_current_season = mysqli_query($connection, $query_current_season);
						
							if ($result_current_season && mysqli_num_rows($result_current_season) == 1) {
								$row = mysqli_fetch_assoc($result_current_season);
								$current_term = $row['current_term']; // Get the current term
								$current_session = $row['current_session']; // Get the current session
							} else {
							...
							// Concatenate student name from firstname, lastname, and othername
							$student_name = ucfirst($student_firstname) . ' ' . ucfirst($student_lastname) . ' ' . ucfirst($student_othername);
						
							// Determine the appropriate table based on student class
							$course_table = ($student_class === "JSS1" || $student_class === "JSS2" || $student_class === "JSS3") ? "jss" : "sss";
						
							// Fetch all required courses for the class
							$query_required_courses = "SELECT * FROM `{$course_table}`";
							$result_required_courses = mysqli_query($connection, $query_required_courses);
						
							if ($result_required_courses && mysqli_num_rows($result_required_courses) > 0) {
								$all_courses_registered = true;
						
								while ($course = mysqli_fetch_assoc($result_required_courses)) {
									$subject_name = $course[$course_table . '_subjects']; // e.g., 'jss_subjects' or 'sss_subjects'
						
									// Check if the subject is already registered for the student
									$check_query = "SELECT * FROM `subjects` WHERE `reg_number` = '{$student_reg_number}' AND `class` = '{$student_class}' AND `subjects` = '{$subject_name}'";
									$check_result = mysqli_query($connection, $check_query);
						
									if (mysqli_num_rows($check_result) == 0) {
										$all_courses_registered = false;
						
										// Insert subject into the 'subjects' table, ensuring name is also saved
										$insert_query = "INSERT INTO `subjects` (`reg_number`, `name`, `class`, `subjects`, `term`, `session`, `registration_date`) 
														 VALUES ('{$student_reg_number}', '{$student_name}', '{$student_class}', '{$subject_name}', '{$current_term}', '{$current_session}', '{$registration_date}')";
										mysqli_query($connection, $insert_query);
									}
								}
						
								// Show appropriate message based on registration status
								if ($all_courses_registered) {
									echo "<p class='text-info'><b>All courses are already registered.</b></p>";
								} else {
									echo "<p class='text-success'><b>All required courses have been successfully registered.</b></p>";
								}
							} else {
								echo "<p class='text-danger'><b>No courses found for the class {$student_class}!</b></p>";
							}
							...
							echo "
								<div class='table-responsive'>
									<table class='table table-striped'>
										<caption class='text-warning'><b>REGISTERED SUBJECTS</b></caption>
										<thead>
											<tr class='info'>
												<th>S/N</th>
												<th>SUBJECTS</th>
							...
							$query_registered = "SELECT * FROM `subjects` WHERE `reg_number` = '{$student_reg_number}' AND `class` = '{$student_class}'";
							$result_registered = mysqli_query($connection, $query_registered);
							...
							if ($result_registered && mysqli_num_rows($result_registered) > 0) {
								...
								echo "<tr><td colspan='8' class='text-danger'>No subjects registered.</td></tr>";
							}
