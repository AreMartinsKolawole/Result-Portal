...
function confused ($get){
    return( sha1(sha1(sha1(htmlentities(urlencode($get))))) );
}
...
function subject_position($position){
    ...
    // If when the position is reversed and the first integer is 1, it should append "st" to the position
    elseif ($first_number_reversed == 1) {
        $position_in_class = "<b>{$position}st</b>";
        return $msg = $position_in_class;
    }
    // If when the position is reversed and the first integer is 2, it should append "nd" to the position
    elseif ($first_number_reversed == 2) {
        $position_in_class = "<b>{$position}nd</b>";
        return $msg = $position_in_class;
    }
    // If when the position is reversed and the first integer is 3, it should append "rd" to the position
